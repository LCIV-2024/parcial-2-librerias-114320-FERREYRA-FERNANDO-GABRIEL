...
public class ExternalBookService {
    ...
    public List<ExternalBookDTO> fetchAllBooks() {
        try {
            if (externalApiUrl == null || externalApiUrl.isBlank()) {
                throw new RuntimeException("La URL de la API externa no está configurada");
            }
            ...
            log.info("Successfully fetched {} books from external API",
            ...
            throw new RuntimeException(
                    "Error al obtener libros de la API externa: " + e.getMessage(), e);
        }
    }
    ...
    public ExternalBookDTO fetchBookById(Long id) {
        try {
            ...
            log.info("Fetching book with id {} from external API: {}", id, url);

            ExternalBookDTO book = restTemplate.getForObject(url, ExternalBookDTO.class);

            log.info("Successfully fetched book: {}",
            ...
            throw new RuntimeException(
                    "Error al obtener el libro de la API externa: " + e.getMessage(), e);
        }
    }
}
